...
*/
   ...
   */
  {
	...
	lprint("Warning: all options past -l are ignored\r\n");
      ...
      if (slirp_socket_passwd) {
	/* Internet connection */
	snprintf(buff, sizeof(buff)-1, "%d %d %s", unit, 0, slirp_socket_passwd);
      }
#ifndef NO_UNIX_SOCKETS
      else {
	snprintf(buff, sizeof(buff)-1, "%d %d %s", unit, (int)getpid(), ttyname(0));
      }
#endif
...
#ifndef NO_UNIX_SOCKETS
		struct sockaddr_un sock_un;
		unsigned int sock_len = sizeof(struct sockaddr_un);
#endif
		struct sockaddr_in sock_in;
		unsigned int sock_len2 = sizeof(struct sockaddr_in);
