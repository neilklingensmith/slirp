    ...
    struct socket *inso;
{
    ...
    } else if (sscanf(buff, "%256[^:]:%256s%*[to \t]%256s", str3, str4, str2) == 3) {
        port = atoi(str4);
        inet_aton(str3, &hostaddr);
        if (inso)
           laddr = inso->so_laddr.s_addr;
        else
           laddr = inet_addr(CTL_LOCAL);
    ...
    if (proto_tcp) {
        so = solisten(hostaddr.s_addr, htons(port), laddr, htons(lport), once_time);
        ...
        if (so)
           ...
           lprint("Redirection failed: %s\r\n", strerror(errno));
                   lprint("Attempted to redirect port %d to %s:%d\r\n", port, laddr, lport);
                }
    } else {
